...
1) Load ((prices)) to df on [md + btd] + cleaning
2) opstrat(prices,start,end,btd) --> ((opts))
3) create associating columns and merge to form ((data))

    - deg
    - win
    - grad
    - acc
    - signal (careful check)
    - equity
    - model_price ?
    - pct_ch_price
    - pct_ch_eq
...
4) Iterate through ((data)) and check for conditions on:
    
    - equity
    - model_price

    These variables can come from a command line arg maybe?
    
    At each step we add that line to a reindexed_like df ((final))
    until the conditions are met. When conditions are met - 
    retreive hour in question - nex. 
    
    nex becomes start:end params for self-iterating/self-eating func?
...
def opstrat(p,ind,bstart,bend,btd,eq,wins,degs):
...
def populate_columns(p,opst,eq,start,end,ind):
    p = p[start:end]
    cols = p.columns
    w = opst['win']
    d = opst['deg']
...
def main(c,md,btd):
    ...
    opst = opstrat(p,ind,istart,iend,btd,eq,ws,ds) # (2) opstr
    
    # M_end is major end, iend is the end of the initial backtest
    bigpos = pd.DataFrame()
    data,posdf = populate_columns(p,opst,eq,iend,M_end,ind)
    ...
    while i != max(f.index):
        if data.loc[i]['rolling_max']==data.loc[i]['rolling_min']==data.loc[i]['equity']:
            f.loc[i] = data.loc[i]
        elif data.loc[i]['rolling_min'] > data.iloc[0]['equity']*0.975:
            f.loc[i] = data.loc[i]
        elif (data.loc[i]['equity'] < data.loc[i]['rolling_max']*0.99 and data.loc[i]['equity']>data.iloc[0]['equity']*1.02) or \
            ...
            print('in else',data.loc[i]['equity'] < data.loc[i]['rolling_max']*0.95,data.loc[i]['equity'],data.loc[i]['rolling_max']*0.95)
            f.loc[i] = data.loc[i]
            # f = add_metrics(f)
        i = i+timedelta(hours=1)
    f = add_metrics(f)
    bigpos['colour'] = [None for _ in range(bigpos.shape[0])]
    for i,x in bigpos.iterrows():
        if x['signal']==1:
            x['colour'] = 'blue'
        elif x['signal']==-1:
            x['colour'] = 'red'
