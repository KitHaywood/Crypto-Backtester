...
def get_initial_opter_strat(indics,prices,wins,degs,istart,iend):
    ...
    def maximiser(series):
        ...
        return series['Equity Final [$]'] if 0 <= series['# Trades'] <= max_trades else 0
...
def plot(crypto,data,df,indics,iopst,eqdf,md):
    ...
    fig.update_layout(title_text=f'{c} - Out-of-Sample 1-Opter Backtest - depth {md} hrs - bt-depth {btd} hrs - max_bt_trades - {max_trades}')    
    fig.show()
...
if __name__=="__main__":
    ...
    # THOMAS - Please give the following a read. 
    
    # To run this script you'll to pip install requirements in a venv. 
    
    # This script is a 'viewer', an out-of-sample backtester. It performs the following
    # to calculate a set of signals the resulting equity had these trades been actioned.
    
    #   Performs an 'in-sample' backtest to retreive the initial optimal params. 
    #   This bt is an exhaustive grid search on 192 possible combinations as - 
    
    #   degs = [4,17,1]
    #   windows = [100,1500,100]
    
    #   The indicators are precomputed and stored in a Gcloud DB, which the script collects from
    #   and then prepares for the backtest optimiser. The indicators are the gradient and acceleration (grad/acc)
    #   i.e. d/dx & d2/dx^2 of the fit-coefficients at sequential hours.
    
    #   This initial backtest is performed over a number of hours - set below in the 'btd' variable.
    # 
    #   Once the initial optimal strategies have been retreived, the scipt then iterates through a set
    #   of prices with corresponding indicator values (this is our 1/192 selected optimal), 
    #   the size of this dataframe is set in the 'd' variable below (hrs). It evaluates buy and sell signals
    #   based on whether the grad/acc behave as:
    #   
    #   Buy if grad crosses through 0 from beneath and acc is already positiv
    #   Sell if grad crosses throug 0 from above and acc is already negative
    
    #   Once the signals are created, the equity is evaluated and resulting sets of data are plotted
    #   for analysis. 

    #   To clarify on timelines. 
    #   The total backtest depth is (initial backtest depth + major backtest depth)
    #   In the below variable defs that is 1500+250. 
    #   There is only so much data precomputed on the DB so try and keep things inside 2500hrs, all data 1hr resolution.
    #   There are 3 instruments currently modelled - BTC, REP XLM. Please interchange in 'c' as you see fit. 
    
    #   To clarify, a good test to check the connection objects are working is to do
    #   from utils import *
    #   in ipython and then see if there is con and db in global dir()
    
    
    global max_trades,btd
    c = "REP"
    max_trades = 10
    btd = 250
    d = 1500
